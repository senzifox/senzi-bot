...
def _is_fresh(cached: tuple[dict[str, float], float] | None) -> bool:
    return cached is not None and time.monotonic() - cached[1] < CACHE_TTL_SECONDS


async def _fetch_rates(session: aiohttp.ClientSession, base_code: str) -> dict[str, float]:
    cached = _rates_cache.get(base_code)
    if _is_fresh(cached):
        return cached[0]

    lock = _fetch_locks.setdefault(base_code, asyncio.Lock())
    async with lock:
        cached = _rates_cache.get(base_code)
        if _is_fresh(cached):
            return cached[0]

        async with session.get(RATES_API_URL.format(base=base_code)) as response:
...
async def convert_currency(
    ...
    rates = await _fetch_rates(session, from_code)
